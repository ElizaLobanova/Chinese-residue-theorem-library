...
Расширенный алгоритм Евклида \ref{algo:Evclid} \cite{Okulov2011} является продолжением общего алгоритма Евклида. 
...
В расширенном алгоритме Евклида добавляются коэффициенты Безу $^1B_{i+1} = ^1B_{i-1} - ^1B_{i}$ и $^2B_{i+1} = ^2B_{i-1} - ^2B_{i}$. Для поиска первого коэффициента подойдёт алгоритм (алг. \ref{algo:Evclid}).
...
Определим операции сложения и вычитания для полученного набора остатков от деления
