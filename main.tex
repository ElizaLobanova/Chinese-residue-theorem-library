...
Основой алгоритма является Китайская теорема об остатках \cite{Cormen2001}:
...
\begin{equation}
	...
	\label{eq:collary}
\end{equation}
 Таким образом чтобы сохранить скажем $2^{20}$ в виде остатков можно было бы взять 2 ближайших простое числа, однако задачей является сохранение в виде чисел размер которых оптимален для работы конкретной архитектуры процессоров, а значит будем брать ближайшие слева числа к $2^{32}$ или $2^{64}$ в количестве $k$ необходимом, для соблюдения условия \ref{eq:collary} 

\subsection{Расчёт результата из массива остатков от деления на простые числа}

Для того чтобы расшифровать число обратно воспользуемся следующим алгоритмом:
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\STATE {Используя расширенный алгоритм Евклида \cite{Okulov2011} найти первый коэффициент Безу $s_i$ для наибольшего общего делителя от $M_i$ и $a_i$:}
		\STATE{Рассчитать искомое число $x = \sum\limits_{i=1}^{k} r_i \cdot M_i \cdot s_i$}
	\end{algorithmic}
	\caption{Расшифровка числа из простых чисел и остатков от делений на них.}
	\label{algo:crt_decription}
\end{algorithm}

Расширенный алгоритм Евклида \ref{algo:Evclid} \cite{Okulov2011} является продолжением общего алгоритма Евклида. В общем алгоритме рассчитываются частный и остатки такие что 

\begin{algorithm}[H]
	\textbf{Ввод:} Два натуральных числа $a$ и $b$.\\
	\textbf{Вывод:} Первый коэффициент Безу.
	\begin{algorithmic}
		\STATE {Рассчитать $M = \prod\limits_{i=1}^k a_i$}
		\STATE {Рассчитать $M_i = \frac{M}{a_i}$}
		\WHILE {r > 0}
...
Определим математические операции для 
