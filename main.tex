...
\affiliationeng[1]{Department of theoretical physics and intellectual technologys, Institute of scientific and advanced materials, Far eastern federal university,\protect\\ 690922, Russia, Vladivostok, Russky Island, Ajax 10}
\affiliationeng[2]{Institute of Applied Mathematics, Far Eastern Branch, Russian Academy of Sciences,\protect\\ 690041, Russia, Vladivostok, Radio 7}
...
Реализация данного подхода требует решения трёх взаимосвязанных задач: выбора системы взаимно простых модулей, обеспечивающей необходимый динамический диапазон представимых чисел; организации арифметических операций над массивами вычетов; а также восстановления результата в позиционной системе счисления. Эти этапы образуют полный вычислительный цикл работы с числами большой разрядности в рамках предложенного метода.
...
\subparagraph{Разбиение большого числа на набор отстатков от делений}

Согласно китайской теореме об остатках \cite{Cormen2001, Sizyi2008}, пусть $a = a_1 a_2 ... a_k$, где $a_i$ попарно взаимно простые натуральные числа, $r = r_1 r_2 ... r_k$ -- натуральные числа удовлетворяющие неравенству $0 \leq r_i < a_i$. Тогда найдётся такое число $L$, для которого для всех $i$ выполняется:
...
Из теоремы следует, что для представления числа $X$ необходимо вычислить его остатки по набору попарно взаимно простых модулей $a_1,a_2,\ldots,a_k$. Простота самих модулей не является обязательным условием: требуется лишь выполнение соотношения

\begin{gather}
    \gcd(a_i,a_j)=1,\qquad i\ne j.
\end{gather}

Произведение модулей определяет динамический диапазон системы остатков:
...
В настоящей реализации в качестве модулей используются различные простые числа. Такой выбор автоматически обеспечивает их попарную взаимную простоту и упрощает формирование модульного базиса. Модули выбираются максимально близкими к верхней границе используемого типа данных до тех пор, пока их произведение $M$ не превысит требуемый диапазон представляемых чисел.

В качестве модулей также могут использоваться степени простых чисел $a_i=p_i^{s_i}$ при условии, что основания $p_i$ для различных модулей не совпадают. Например, числа $125=5^3$ и $121=11^2$ являются взаимно простыми и могут одновременно входить в модульный базис. Напротив, различные степени одного простого числа не являются взаимно простыми и не могут одновременно использоваться в классической форме КТО.
...
Второй подход состоит в использовании модулей, остатки по которым могут храниться в компактном типе данных, например \texttt{int8}. Это позволяет уменьшить объём памяти, необходимый для хранения модульного представления, и снизить нагрузку на подсистему памяти GPU. При использовании знакового типа \texttt{int8} в данной реализации выбираются положительные модули $a_i < 128$. Промежуточные результаты арифметических операций вычисляются в типе большей разрядности, после чего приводятся по соответствующему модулю.

\subparagraph{Математические операции с массивами остатков}
...
\end{align}
то есть, чтобы сложить, вычесть или умножить два числа, достаточно сложить, вычесть или умножить соответствующие элементы векторов остатков этих двух чисел.

Поскольку $r_i \in (\mathbb{N} \cup 0)$, операция деления не определена для модульной арифметики в явном виде: целочисленное деление нельзя выполнить независимо над отдельными остатками, поскольку представление в системе остаточных классов не сохраняет информацию о порядке и величине чисел. Для выполнения деления требуются специализированные алгоритмы либо восстановление исходных чисел. Это ограничение накладывает определённые сложности на использование КТО в задачах, где деление является ключевой операцией. В таких случаях необходимо либо использовать дополнительные методы для восстановления исходных чисел, либо применять алгоритмы, которые обходятся без явного деления.

\subparagraph{Восстановление числа из массива остатков}

Для восстановления исходного числа необходимо для каждого $i=1,\ldots,k$ найти число

\begin{gather}
    ^1B_i=M_i^{-1}\bmod a_i,
\end{gather}
то есть модульно обратный элемент для $M_i$ по модулю $a_i$. Поскольку модули $a_1,\ldots,a_k$ попарно взаимно просты, выполняется
...
Следовательно, обратный элемент $^1B_i$ существует.

Для его вычисления применяется расширенный алгоритм Евклида \cite{Okulov2011}. Для двух взаимно простых натуральных чисел $u$ и $v$ алгоритм строит последовательности $R_j$, $S_j$ и $T_j$, для которых на каждой итерации выполняется равенство
...
\begin{gather}
    R_0=u,\quad R_1=v,\qquad
    S_0=1,\quad S_1=0,\qquad
    T_0=0,\quad T_1=1.
\end{gather}

На каждой итерации вычисляется целая часть частного

\begin{gather}
    q_j=\left\lfloor\frac{R_{j-1}}{R_j}\right\rfloor,
\end{gather}
после чего последовательности обновляются согласно Алгоритму \ref{algo:Evclid}. В рассматриваемой задаче полагается $u=M_i$ и $v=a_i$.

\begin{algorithm}[H]
    \textbf{Ввод:} Взаимно простые натуральные числа $u$ и $v$.\\
    \textbf{Вывод:} Обратный элемент $^1B=u^{-1}\bmod v$.
    \begin{algorithmic}
        \STATE {$R_0=u$, $R_1=v$}
        \STATE {$S_0=1$, $S_1=0$}
        \STATE {$T_0=0$, $T_1=1$}
        ...
        \WHILE {$R_j\ne 0$}
            \STATE {$q_j=
                \left\lfloor R_{j-1}/R_j\right\rfloor$}
            \STATE {$R_{j+1}=R_{j-1}-q_jR_j$}
            \STATE {$S_{j+1}=S_{j-1}-q_jS_j$}
            \STATE {$T_{j+1}=T_{j-1}-q_jT_j$}
            \STATE {$j=j+1$}
        \ENDWHILE
        \STATE {$^1B=S_{j-1}\bmod v$}
    \end{algorithmic}
    \caption{Вычисление обратного элемента с использованием
    расширенного алгоритма Евклида.}
    \label{algo:Evclid}
\end{algorithm}

Для того чтобы расшифровать число обратно используется Алгоритм \ref{algo:crt_decription}.
...
\begin{algorithm}[H]
	...
	\begin{algorithmic}
		...
		\STATE {Для каждого $i=1,\ldots,k$ с использованием Алгоритма \ref{algo:Evclid} рассчитать $^1B_i=M_i^{-1}\bmod a_i$}
		\STATE {Рассчитать искомое число $x=\left(\sum\limits_{i=1}^{k}r_iM_i{}^1B_i\right)\bmod M$}
	\end{algorithmic}
	\caption{Восстановление числа из простых чисел и остатков от делений на них.}
	\label{algo:crt_decription}
\end{algorithm}

В Алгоритме \ref{algo:crt_decription} для каждого модуля $a_i$ вычисляется произведение остатка $r_i$, величины $M_i$ и соответствующего первого коэффициента Безу ${}^1B_i$. Сумма полученных произведений, взятая по модулю $M$, равна исходному числу.
...
\bibitem[Cormen et al., 2022]{Cormen2001} {\it Cormen~T.\,H., Leiserson~C.\,E., Rivest~R.\,L., Stein~C.} Introduction to algorithms.~--- MIT Press, 2022.\\

\bibitem[Сизый, 2008]{Sizyi2008}
{\it Сизый~С.\,В.} Лекции по теории чисел.~--- 2-е изд., испр.~--- М.: ФИЗМАТЛИТ, 2008.~--- 192~с. //
{\footnotesize{\it Sizyi~S.\,V.} Lektsii po teorii chisel [Lectures on number theory].~--- 2nd ed., revised.~--- Moscow: FIZMATLIT, 2008.~--- 192~p. (in Russian).\par}
