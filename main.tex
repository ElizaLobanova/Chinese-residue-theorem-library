...
The article explores the application of the Chinese Remainder Theorem (CRT) to optimize computations involving large numbers in supercomputing, particularly in GPU environments. The proposed method involves decomposing large numbers into remainders with respect to a set of prime numbers, which facilitates efficient arithmetic operations such as addition, subtraction, and multiplication while mitigating the risk of memory overflow. The advantage of this approach lies in its parallelization potential, which significantly enhances computational speed and reduces memory load. However, reconstructing the original number from the remainders is resource-intensive, potentially diminishing overall performance. The paper concludes that CRT is a powerful tool for parallel computing, offering solutions to memory limitations and overflow issues, although it may introduce some complexity during number reconstruction.

\keywords{multi-precision arithmetic, supercomputers, modular arithmetic, parallelization}
...
\keywords{большие числа, суперкомпьютеры, модульная арифметика, параллелизм.}
...
\section{Алгоритм работы с большими числами} 
...
Предлагаемый подход обладает рядом преимуществ, которые делают его эффективным для использования в вычислениях на GPU. Во-первых, независимый параллелизм вычислений с использованием алгоритма КТО достигается за счёт работы с отдельными остатками. Это существенно ускоряет обработку массивов остатков на разных потоках или ядрах GPU.
